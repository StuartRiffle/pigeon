// book.h - PIGEON CHESS ENGINE (c) 2012-2016 Stuart Riffle

namespace Pigeon {
#ifndef PIGEON_BOOK_H__
#define PIGEON_BOOK_H__

// This is pretty hacky, and eventually to be replaced with a proper opening book of some sort.
// The list below was generated by filtering a file of ECO lines to remove very short ones. The book
// move at a given position is chosen randomly, but in proportion to the number of book lines
// that use it.
...
class OpeningBook
{
    typedef std::map< std::string, int > BookMoveCount;

    struct BookNode
    {
        BookMoveCount   mMoves;
        int             mTotalLines;

        BookNode() : mTotalLines( 0 ) {}
    };

    typedef std::map< u64, BookNode > BookNodeMap;

    BookNodeMap         mBookNode;

    INLINE u64 GetPositionKey( const Position& pos ) const
    {
        return( pos.mHash ^ pos.mWhiteToMove );
    }
...
public:
    void Init()
    {
        ...
        for( int onLine = 0; sBookLines[onLine]; onLine++ )
        {
            Tokenizer tokens( sBookLines[onLine] );
            Position  pos;
            ...
            {
                u64 key = this->GetPositionKey( pos );

                BookNode& node = mBookNode[key];
                node.mTotalLines++;
                ...
                if( idx < 0 )
                {
                    printf( "info string ERROR: invalid book line \"%s\"\n", sBookLines[onLine] );
                    break;
                }

                node.mMoves[movetext]++;
    ...
    const char* GetBookMove( const Position& pos )
    {
        MoveSpec result;
        u64 key = this->GetPositionKey( pos );

        BookNodeMap::iterator iterNode = mBookNode.find( key );
        if( iterNode != mBookNode.end() )
        {
